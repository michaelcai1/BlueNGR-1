...
#define portNVIC_SYSTICK_COUNT_FLAG_BIT		( 1UL << 16 )
#define portNVIC_PENDSVCLEAR_BIT 			( 1UL << 27 )
#define portNVIC_PEND_SYSTICK_CLEAR_BIT		( 1UL << 25 )
...
#define portPRIORITY_GROUP_MASK				( 0x07UL << 8 )
...
{
	__asm volatile ("   .extern pxCurrentTCB        \n"
                    "   push {lr}                   \n"
	    			"   ldr	r3, =pxCurrentTCB	    \n" /* Restore the context. */
...
static void prvPortStartFirstTask( void )
{
	__asm volatile(
                    " mov r0, #0x2          \n"
                    " msr control, r0       \n"
					" ldr r0, =0x10040000 	\n" /* locate the stack from vector table. */
					//" ldr r0, [r0] 			\n"
